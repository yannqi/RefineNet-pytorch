"""RefineNet
RefineNet PyTorch for non-commercial purposesd
"""
...
class RefineNet(nn.Module):
    ...
    def forward(self, x):
        ...
        l1 = self.layer1(x) #resnet_101 conv2_x
        l2 = self.layer2(l1) #resnet_101 conv3_x
        l3 = self.layer3(l2) #resnet_101 conv4_x
        l4 = self.layer4(l3) #resnet_101 conv5_x  out_channel = 2048

        l4 = self.dropout(l4)  #dropout 0.5
        l3 = self.dropout(l3)  #dropout 0.5
        # RefineNet-4  without MRF
        x4 = self.p_ims1d2_outl1_dimred(l4)  #TODO why conv3x3?  2048->512
        x4 = self.adapt_stage1_b(x4)  # 2x RCU 
        x4 = self.relu(x4)
        x4 = self.mflow_conv_g1_pool(x4) # CRP
        x4 = self.mflow_conv_g1_b(x4) #TODO 3x RCU   need to change to 1x RCU  
        x4 = self.mflow_conv_g1_b3_joint_varout_dimred(x4)
        x4 = nn.Upsample(size=l3.size()[2:], mode='bilinear', align_corners=True)(x4)
        # RefineNet-3
        x3 = self.p_ims1d2_outl2_dimred(l3)
        x3 = self.adapt_stage2_b(x3)
        x3 = self.adapt_stage2_b2_joint_varout_dimred(x3)  #MRF
        x3 = x3 + x4
        x3 = F.relu(x3)
